...
@Component({
  selector: 'app-patient-preferences-dropdown',
  templateUrl: './patient-preferences-dropdown.component.html',
  styleUrls: ['./patient-preferences-dropdown.component.css']
})
export class PatientPreferencesDropdownComponent implements OnInit {

  @Input() options: any;
  @Input() commonPlaceholder: any;
  @Output() selectOptionvalue = new EventEmitter();
  ...
  ngOnInit() {
	this.selectOptionvalue.emit(this.commonPlaceholder);

	this.UWlistArraylength=this.options.length;
  ...
  initializeDropdown() {
	...
	this.ddplaceholder = this.commonPlaceholder;
  ...
  selectDdOption(indx){
	...
	this.selectOptionvalue.emit(this._ddoptionsArr[indx].option);
  }
